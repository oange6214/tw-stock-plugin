...
### 訊號三：底部站回 MA20
> 專為本流程設計：進入此流程的股票長期在 MA20 以下，剛站回正乖離
- 過去多數時間在 MA20 以下（已由 Phase 2 確認）
- 近期收盤站回 MA20 之上（乖離率 0~5%）
- 近 3 日量能較前期放大，或量比 ≥ 1.0x（價漲量撐）
- 收盤站穩 MA20 上方（未出現收盤跌回 MA20 以下）

填入 `entry_signal`：`"breakout"` / `"pullback"` / `"ma20_reclaim"`
...
- **技術位階停損**：訊號一 → 突破那根紅K的低點；訊號二 → 跌破支撐均線（10MA 或 20MA）；訊號三 → 收盤跌回 MA20 以下
- **固定比例停損**：進場價 × 0.93（-7%）
- 取兩者較高價（較嚴格）為最終停損點
