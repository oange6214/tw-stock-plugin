...
直接在主對話取得大盤資料。優先呼叫 MCP 工具 `get_market_overview`；若 MCP 工具未在本 session 載入，改用以下備援方式：

```bash
# 備援：直接呼叫 TWSE OpenAPI
curl -s "https://openapi.twse.com.tw/v1/exchangeReport/MI_INDEX" -o /tmp/mi_index.json
# 再用 Python 解析（需指定 utf-8）
```

**不要啟動子 agent**，子 agent 無法存取 MCP 工具。
